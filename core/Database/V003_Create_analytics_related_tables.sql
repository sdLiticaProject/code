...
CREATE TABLE `ANALYTICS_MODULES_OPERATIONS` (
  `ID` VARCHAR(36) NOT NULL,
  ...
  CONSTRAINT `MODULESOPERATIONS_OPERATIONS_FK`
    ...
    REFERENCES `ANALYTICS_OPERATIONS` (`ID`)
    ON DELETE CASCADE,
  CONSTRAINT `MODULESOPERATIONS_MODULES_FK`
    FOREIGN KEY (`ANALYTICS_MODULE_ID`)
    REFERENCES `ANALYTICS_MODULES` (`ID`)
    ON DELETE CASCADE
) ENGINE = InnoDB;

CREATE TABLE `TIMESERIES_ANALYTICS_OPERATIONS` (
  `ID` VARCHAR(36) NOT NULL,
